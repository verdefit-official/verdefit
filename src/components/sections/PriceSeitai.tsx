...
function CouponCard({ coupon, i }: { coupon: CouponItem; i: number }) {
  return (
    <div key={coupon._key ?? i} className="relative rounded-xl bg-white px-7 py-8 text-center">
      {coupon.badge && (
      ...
      <p className="mt-3 whitespace-pre-line text-xs text-gray-500">{coupon.validity}</p>
    </div>
  );
}
